...
Portless runs a reverse proxy on port 80 (or 443 with `--https`). Each app registers a route mapping its hostname to an assigned port. Requests to `http://<name>.localhost` are proxied to the app.

```
Browser (myapp.localhost) -> proxy (port 80 or 443) -> App (random port)
```
